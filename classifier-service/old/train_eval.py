...
mainnet data (bot_like / human_like). No synthetic data is used anywhere
in this pipeline; see fetch_real_data.py and extract_features_real.py for
how the real, heuristically-labeled dataset is built.
...
-----
    ...
    python3 classifier-service/train_eval.py --min-tx 5   # drop short-history rows
"""
import argparse
import json
import os
...
MODEL_OUT_PATH = "classifier-service/model.joblib"
REAL_FEATURES_PATH = "classifier-service/real_features.json"
...
def load_real_data(min_tx=0):
    if not os.path.exists(REAL_FEATURES_PATH):
        raise SystemExit(
            ...
            f"fetch_real_data.py to collect the raw addresses)."
        )
...
def main():
    ...
    print("\nFull results written to classifier-service/eval_results.json")
    with open("classifier-service/eval_results.json", "w") as f:
    ...
    bundle = {
        ...
        "notes": (
            ...
            "extract_features_real.py / fetch_real_data.py). Predict with "
    ...
    fig.tight_layout()
    fig.savefig("classifier-service/pca_plot.png", dpi=150)
