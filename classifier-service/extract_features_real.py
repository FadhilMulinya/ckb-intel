"""
Turns the real address data collected by fetch_real_data.py into feature
rows, by calling extract_features.extract_features() directly (not
reimplementing it), so training and inference always share one feature
definition.
...
Output rows carry `label_source` ("bot_like" / "human_like") and
`data_origin: "real_mainnet"`, so real data can never be silently
confused with any other source downstream.
"""
import argparse
import json
import os
...
def main():
    ...
    ap.add_argument("--out", default="classifier-service/real_features.json")
    args = ap.parse_args()
