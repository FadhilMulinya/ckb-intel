...
from wallet_intelligence.features_v2.config import (DATASET_VERSION, FEATURE_CONFIG_VERSION,
                                FEATURE_SCHEMA_VERSION, OBSERVATION_CONTRACT_VERSION)
from wallet_intelligence.features_v2.pipeline import assess_observation_v2, load_observation_v2
from wallet_intelligence.features_v2.stats import pearson, quantile

ROOT = Path(__file__).resolve().parents[2]
DATA_DIR = ROOT / "ckb_data"
DEFAULT_DB = DATA_DIR / "ckb_data_v2" / "ckb_explorer.sqlite"
V1_DIR = DATA_DIR / "dataset_completion"
OUT_DIR = DATA_DIR / "feature_engineering_v2"
...
def unresolved_assessment(reason: str) -> dict:
    from wallet_intelligence.features_v2.config import MINIMUM_SAMPLES
    from wallet_intelligence.features_v2.rules import RULE_NAMES, RULE_VERSION
    families = {}
...
def feature_definitions() -> dict:
    from wallet_intelligence.features_v2 import capacity, lifecycle, lineage, periodicity, scripts, templates, temporal, topology, typed_assets
    definitions = {
    ...
    for family, definition in definitions.items():
        ...
        definition["executable_definition"] = f"classifier-service/wallet_intelligence/features_v2/{family}.py::extract"
