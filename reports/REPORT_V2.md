...
> heuristic Random Forest classifier documented in the Git history (the
> legacy/abandoned baseline). The executable baseline was removed; this report
> is not an identity-ground-truth report. See §9 below for the methodological
> comparison.
...
**Predecessor report:** historical V1 report in Git history <br>
...
pipeline that has replaced the proxy-label classifier in the historical V1 baseline. The
...
- **Data source:** cached CKB Explorer records labelled mainnet, fixed 30-day observation window
...
| | the historical V1 baseline (legacy) | This report (current) |
|---|---|---|
...
- Dataset: `ckb-behaviour-dataset-v1`; population version
  `ckb-wallet-population-local-v1`; authoritative manifest
  `ckb_data/dataset_completion/population_manifest_v1.jsonl`; manifest hash
...
- 51,816 wallet-transaction participation rows (47,145 distinct transaction
  hashes); of 92,515 total input rows, 56,407 are applicable (non-cellbase)
  previous-output rows and all have `resolution_status=complete`. The
  dedicated normalized `previous_output_index` column is present but null in
  the frozen rows; the value is recovered from cached raw JSON by the V2
  loader where available.
...
| Criterion | Verdict |
|---|---|
| Data collection quality | ⚠️ QUALIFIED — applicable rows have complete resolution status; normalized output indices and lock-script rows remain incomplete |
...
| Reproducible | ✅ YES, with qualified test reporting | Offline hashes/contracts pass; the actual suite is 71 passing and 11 skipped |
...
## 9. Comparison with the legacy baseline (the historical V1 baseline)

### 9.1 Abandoned approach (the historical V1 baseline)
...
- ✅ Reproducible from frozen Explorer data labelled mainnet
...
5. Do not reuse the historical V1 baseline's heuristic labels for any part of this
