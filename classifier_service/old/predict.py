...
which sits alongside it) so classifier-service/ can be deployed as a standalone
service without needing the rest of the repo at inference time. The HTTP
client here intentionally mirrors fetch_real_data.py's proven
request/retry logic -- if that script's endpoints ever change, update both.

USAGE
-----
    python3 classifier-service/predict.py <ckb-address>
    python3 classifier-service/predict.py <ckb-address> --json

Run from the repo root (same convention as train_eval.py), so the
relative model path resolves correctly.

REGISTRY_SERVICE_URL (optional)
--------------------------------
If set (e.g. "http://localhost:3000"), classify() writes its verdict back
to the registry-service wallet registry after scoring -- see
registry_client.py. This is a pure side effect: transaction fetching above
is completely unchanged, and a write-back failure never fails the
classification itself. Unset by default, so existing callers see zero
behavior change.
"""
import argparse
import json
...
import registry_client  # same-directory import, only used if REGISTRY_SERVICE_URL is set

REGISTRY_SERVICE_URL = os.environ.get("REGISTRY_SERVICE_URL")  # e.g. "http://localhost:3000"
...
# working directory -- the old "classifier-service/model.joblib" only worked if
...
def _maybe_write_back(address, result):
    """Best-effort: if REGISTRY_SERVICE_URL is set, push this verdict to
    registry-service so it's queryable there. Never raises -- a write-back
    failure must not affect the classification response the caller
    actually asked for."""
    if not REGISTRY_SERVICE_URL:
        return
    try:
        registry_client.write_back_label(
            address, REGISTRY_SERVICE_URL, result["verdict"], result.get("bot_probability")
        )
    except Exception as e:  # noqa: BLE001 -- deliberately broad, this is a side effect
        print(f"NOTE: registry-service write-back failed for {address}: {e}", file=sys.stderr)
...
def classify(address, model_path=MODEL_PATH, max_tx=300):
    ...
    if n_tx < 2:
        ...
        _maybe_write_back(address, result)
        return result
